"""
LIKE V5, but fixes wind direction bug by storing raw positions + wind direction
in replay buffer and transforming at sample time.
"""
...
def get_env_wind_directions(envs, num_envs: int) -> np.ndarray:
    ...
    """
    wind_dirs = np.zeros(num_envs, dtype=np.float32)
    
    if hasattr(envs, 'envs'):
        for i in range(num_envs):
            base_env = envs.envs[i]
            if hasattr(base_env, 'mean_wind_direction'):
                # RandomLayoutEnv
                wind_dirs[i] = base_env.mean_wind_direction
            elif hasattr(base_env, 'wd'):
                # Single layout env (possibly wrapped)
                wind_dirs[i] = base_env.wd
            else:
                # Try to unwrap
                env = base_env
...
def get_env_raw_positions(envs, num_envs: int, n_turbines_max: int) -> np.ndarray:
    ...
    """
    positions = np.zeros((num_envs, n_turbines_max, 2), dtype=np.float32)
    ...
    if hasattr(envs, 'envs'):
        for i in range(num_envs):
            ...
            if hasattr(base_env, 'turbine_positions'):
                # RandomLayoutEnv - already padded
                positions[i] = base_env.turbine_positions
            else:
                # Single layout - need to extract and pad
                env = base_env
                while hasattr(env, 'env'):
                    env = env.env
                
                if hasattr(env, 'x_pos') and hasattr(env, 'y_pos'):
                    pos = np.column_stack([env.x_pos, env.y_pos])
                    n_turb = pos.shape[0]
                    positions[i, :n_turb] = pos
    
    return positions


def get_env_attention_masks(envs, num_envs: int, n_turbines_max: int, use_random_layouts: bool) -> np.ndarray:
    """
    ...
    Args:
        ...
        use_random_layouts: Whether using random layouts mode
        
    Returns:
        masks: (num_envs, n_turbines_max) boolean array, True = padding
    """
    if not use_random_layouts:
        # Single layout - no padding needed
        return np.zeros((num_envs, n_turbines_max), dtype=bool)
    
    masks = np.zeros((num_envs, n_turbines_max), dtype=bool)
    
    if hasattr(envs, 'envs'):
        for i in range(num_envs):
            base_env = envs.envs[i]
            if hasattr(base_env, 'attention_mask'):
                masks[i] = base_env.attention_mask
...
if __name__ == "__main__":
    ...
    for update in range(num_updates + 2):
        ...
        current_masks = get_env_attention_masks(envs, args.num_envs, n_turbines_max, use_random_layouts)
