"""
Transformer-based SAC for Wind Farm Control - V13

Changes in V14:
- Added option to run evaluations during training (--eval_interval, --num_eval_steps --num_eval_episodes)

Changes in V13:
- Removed farm token option for simplicity
- Made the use_wd_deviation flag so we can use absolute WD if desired
...
class Args:
    ...
    use_wd_deviation: bool = False  # NEW: If True, convert WD to deviation from mean
...
class TransformerActor(nn.Module):
    ...
    def forward(
        ...
        if self.uses_rope:
            h, attn_weights = self.transformer(h, positions_for_rope, key_padding_mask)
        else:
...
class TransformerCritic(nn.Module):
    ...
    def forward(
        ...
        # Transformer (different call signature for RoPE vs standard)
        if self.uses_rope:
            h, _ = self.transformer(h, positions_for_rope, key_padding_mask)
        else:
