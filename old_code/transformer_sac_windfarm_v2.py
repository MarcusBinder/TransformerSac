...
class Args:
    ...
    dt_env: int = 10
    yaw_step: float = 5.0
    num_envs: int = 2  # Number of parallel environments (only used for single-layout)
...
# =============================================================================
# Episode Statistics Wrapper for Non-Vectorized Environments
# =============================================================================

class RecordEpisodeStatistics(gym.Wrapper):
    """
    A simple wrapper to record episode statistics for non-vectorized environments.
    
    This is used when we can't use the vectorized RecordEpisodeVals wrapper.
    """
    ...
    def step(self, action):
        ...
        # Try to get power from info if available
        if "power" in info:
            self._episode_powers.append(info["power"])
        
        done = terminated or truncated
        if done:
            self.return_queue.append(self._episode_return)
            self.length_queue.append(self._episode_length)
            if self._episode_powers:
                self.mean_power_queue.append(np.mean(self._episode_powers))
            ...
            if self._episode_powers:
                info["episode"]["mean_power"] = np.mean(self._episode_powers)
...
def make_single_env(layout_type: str, args, wind_turbine, seed: int = 0):
    """Create a single environment with the specified layout (with per-turbine wrapper)."""
...
def make_base_env(layout_type: str, args, wind_turbine, seed: int = 0):
    """Create a base environment WITHOUT per-turbine wrapper (for vectorization)."""
    x_pos, y_pos = get_layout_positions(layout_type, wind_turbine)
    config = make_env_config()
...
class MultiLayoutEnv(gym.Env):
    ...
    def __init__(self, layout_types: List[str], args, wind_turbine, seed: int = 0):
        ...
        self.envs = {
            layout: make_single_env(layout, args, wind_turbine, seed)
...
# =============================================================================
# Vectorized Environment Wrapper for Per-Turbine Observations
# =============================================================================

class VectorizedPerTurbineWrapper(gym.Wrapper):
    """
    Wrapper that converts vectorized env observations to per-turbine format.
    
    Takes a vectorized environment and reshapes observations from
    (num_envs, flat_obs_dim) to (num_envs, n_turbines, obs_dim_per_turbine).
    
    This allows using vectorized environments with the transformer architecture.
    """
    
    def __init__(self, env, n_turbines: int, obs_dim_per_turbine: int):
        super().__init__(env)
        self.n_turbines = n_turbines
        self.obs_dim_per_turbine = obs_dim_per_turbine
        self.num_envs = env.num_envs
        
        # Update observation space to per-turbine format
        # Note: This is informational; actual shape depends on how we process
        low = env.single_observation_space.low.reshape(n_turbines, obs_dim_per_turbine)
        high = env.single_observation_space.high.reshape(n_turbines, obs_dim_per_turbine)
        self.single_observation_space = gym.spaces.Box(low=low, high=high, dtype=np.float32)
        
    def reset(self, **kwargs):
        obs, info = self.env.reset(**kwargs)
        # Reshape: (num_envs, flat) -> (num_envs, n_turbines, obs_dim)
        obs = obs.reshape(self.num_envs, self.n_turbines, self.obs_dim_per_turbine)
        return obs, info
    
    def step(self, action):
        # Flatten action if needed: (num_envs, n_turbines, action_dim) -> (num_envs, flat)
        if action.ndim == 3:
...
def get_positions_tensor_from_env_data(positions_np, rotor_d, wind_dir, device, wind_relative: bool = True):
    """
    Get normalized turbine positions from numpy arrays.
    
    For use with vectorized environments where we need to pass explicit data.
    """
    # Normalize by rotor diameter
...
if __name__ == "__main__":
    ...
    # Determine if we're doing multi-layout training (variable farm sizes)
    multi_layout = len(train_layouts) > 1
    ...
    # Create environment(s) based on training mode
    if multi_layout:
        # Multi-layout training: use single non-vectorized env with custom wrapper
        print("Multi-layout training mode: using single environment with variable farm sizes")
        env = MultiLayoutEnv(train_layouts, args, wind_turbine, args.seed)
        env = RecordEpisodeStatistics(env)  # Use our custom non-vector wrapper
        
        # For multi-layout, we use the TransformerReplayBuffer which handles variable sizes
        use_transformer_buffer = True
    else:
        # Single-layout training: can use vectorized environments
        print(f"Single-layout training mode: using {args.num_envs} parallel environment(s)")
        
        layout = train_layouts[0]
        
        if args.num_envs == 1:
            # Single env - use the per-turbine wrapper directly
            env = make_single_env(layout, args, wind_turbine, args.seed)
            env = RecordEpisodeStatistics(env)  # Use our custom non-vector wrapper
            use_transformer_buffer = True
        else:
            # Multiple envs - vectorize
            def make_env_fn(seed):
                def _init():
                    return make_base_env(layout, args, wind_turbine, seed)
                return _init
            ...
            env = RecordEpisodeVals(env)  # Now this works with vectorized env
            
            # Get turbine info for reshaping
            sample_env = make_single_env(layout, args, wind_turbine, args.seed)
            sample_obs, _ = sample_env.reset()
            n_turbines = sample_obs.shape[0]
            obs_dim_per_turbine = sample_obs.shape[1]
            sample_env.close()
            
            # Wrap to reshape observations
            env = VectorizedPerTurbineWrapper(env, n_turbines, obs_dim_per_turbine)
            use_transformer_buffer = False  # Use standard SB3 buffer for vectorized
    
    # Get dimensions from env
    sample_obs, _ = env.reset()
    if sample_obs.ndim == 3:
        # Vectorized: (num_envs, n_turbines, obs_dim)
        n_turbines_sample = sample_obs.shape[1]
        obs_dim_per_turbine = sample_obs.shape[2]
    else:
        # Non-vectorized: (n_turbines, obs_dim)
    ...
    # Replay buffer - always use TransformerReplayBuffer for simplicity with transformer arch
    ...
    # Handle vectorized vs non-vectorized observations
    if obs.ndim == 3:
        # Vectorized: use first env for positions (all same layout)
        num_envs = obs.shape[0]
        # For vectorized single-layout, positions are the same for all envs
        # We need to get positions from the underlying env
        if hasattr(env, 'envs'):
            # AsyncVectorEnv
            base_env = env.envs.call('unwrapped')[0]
            positions = get_positions_tensor_from_env_data(
                np.column_stack([base_env.x_pos, base_env.y_pos]),
                base_env.rotor_D,
                270.0,  # Default wind direction
                device,
                args.use_wind_relative_pos
            )
        else:
            positions = get_positions_tensor(env.env, device, args.use_wind_relative_pos)
    else:
        # Non-vectorized
        num_envs = 1
        if hasattr(env, 'current_env'):
            # MultiLayoutEnv
            positions = get_positions_tensor(env.current_env, device, args.use_wind_relative_pos)
        elif hasattr(env, 'env') and hasattr(env.env, 'turbine_positions'):
            positions = get_positions_tensor(env.env, device, args.use_wind_relative_pos)
        else:
            ...
            positions = get_positions_tensor(base_env, device, args.use_wind_relative_pos)
    
    print(f"\nStarting training for {args.total_timesteps} timesteps...")
    print(f"Using {num_envs} environment(s)")
    ...
    for step in range(args.total_timesteps):
        ...
        if global_step < args.learning_starts:
            ...
            with torch.no_grad():
                ...
                # Expand positions for batch if needed
                if positions.shape[0] == 1 and obs_tensor.shape[0] > 1:
                    positions_batch = positions.expand(obs_tensor.shape[0], -1, -1)
                else:
                ...
                # Flatten for env
                action = action.reshape(-1) if num_envs == 1 else action.reshape(num_envs, -1)
        
        # Step environment
        next_obs, reward, terminated, truncated, info = env.step(action)
        done = terminated if isinstance(terminated, bool) else terminated.any()
        ...
        if num_envs == 1:
            ...
            # For vectorized envs, add each transition separately
            for i in range(num_envs):
                d = terminated[i] or truncated[i]
                action_reshaped = action[i].reshape(-1, action_dim_per_turbine)
                rb.add(obs[i], next_obs[i], action_reshaped, reward[i], d, positions_np)
        
        obs = next_obs
        
        # Handle episode end logging
        if hasattr(env, 'return_queue') and len(env.return_queue) > 0:
            # Check if we have new episode data
            if done or ("final_info" in info if isinstance(info, dict) else False):
                writer.add_scalar("charts/episodic_return", np.mean(env.return_queue), global_step)
                if hasattr(env, 'mean_power_queue') and len(env.mean_power_queue) > 0:
                    writer.add_scalar("charts/episodic_power", np.mean(env.mean_power_queue), global_step)
                print(f"Step {global_step}: return={np.mean(env.return_queue):.2f}")
        
        # Update positions on episode reset (for multi-layout)
        if done and multi_layout:
            if hasattr(env, 'current_env'):
                positions = get_positions_tensor(env.current_env, device, args.use_wind_relative_pos)
