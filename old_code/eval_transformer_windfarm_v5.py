...
from transformer_sac_windfarm_v4 import (
    TransformerActor,
    TransformerCritic,
    # make_single_layout_env,
    # make_random_layout_env,
...
def make_eval_env(layout: str, config: EvalConfig, wind_turbine, seed: int):
    """
    Create an environment for evaluation with the specified layout.
    
    This is a helper that creates the env with the right settings.
    """
    ...
    env = WindFarmEnv(
        ...
        n_passthrough=config.max_eps,
        x_pos=x_pos,
        y_pos=y_pos,
        TurbBox="/work/users/manils/rl_timestep/Boxes/V80env/",
        config=env_config,
        turbtype=config.TI_type,
        dt_sim=config.dt_sim,
        dt_env=config.dt_env,
        yaw_step_sim=config.yaw_step,
    )
...
def get_positions_tensor_for_eval(env, device: torch.device, wind_relative: bool = True):
    """
    Get normalized turbine positions as a tensor for evaluation.
    
    Works with PerTurbineObservationWrapper-wrapped environments.
    """
    # Get positions from the underlying env
    base_env = env
    while hasattr(base_env, 'env'):
        base_env = base_env.env
    
    positions = np.column_stack([base_env.x_pos, base_env.y_pos])
    rotor_d = base_env.D
    ...
    if wind_relative:
        wind_dir = base_env.wd  # Current wind direction
    ...
    return positions_tensor, rotor_d
...
def evaluate_on_layout(
    ...
    """
    env = make_eval_env(layout, config, wind_turbine, config.seed)
    
    # Get base env for properties
    base_env = env
    while hasattr(base_env, 'env'):
        base_env = base_env.env
    
    n_turbines = base_env.n_turb
    rotor_diameter = base_env.D
    ...
    for ep in range(config.n_episodes):
        ...
        positions, _ = get_positions_tensor_for_eval(env, device, wind_relative=True)
        ...
        while not done:
            ...
            if collect_attention and ep < config.plot_attention_episodes:
                ...
                # Get current positions from base env
                current_positions = np.column_stack([base_env.x_pos, base_env.y_pos])
            ...
            # Get power from info if available
            if "Power agent" in info:
                episode_power.append(info["Power agent"])
...
def plot_attention_on_farm(
    ...
    if show_wind_arrow:
        ...
        arrow_start = (pos_norm[:, 0].min() - 0.15 * x_range - 1, 
                       pos_norm[:, 1].max() + 0.1 * y_range + 1)
...
def run_full_evaluation(config: EvalConfig):
    ...
    print(f"Using turbine: {config.turbtype} (D={rotor_diameter:.1f}m)")
    
    # Get dimensions from first layout
    test_env = make_eval_env(config.layouts[0], config, wind_turbine, config.seed)
    sample_obs, _ = test_env.reset()
    obs_dim_per_turbine = sample_obs.shape[1]
    n_turbines_test = sample_obs.shape[0]
    ...
    for layout in config.layouts:
        ...
        if results['mean_power'] is not None:
        ...
        if config.plot_attention and results["attention_data"]:
            ...
            analysis = analyze_attention_vs_wake(all_attention, results["rotor_diameter"])
            ...
            if results["attention_data"]:
                create_attention_visualization_grid(
                    ...
                    rotor_diameter=results["rotor_diameter"],
                    layout_name=layout,
    ...
    with open(os.path.join(config.save_dir, "results.json"), "w") as f:
        json.dump(results_summary, f, indent=2, default=lambda x: None if x != x else x)  # Handle NaN
...
def main():
    ...
    parser.add_argument("--max_eps", type=int, default=20, help="Max episodes per env")
    parser.add_argument("--TI_type", type=str, default="Random", help="Turbulence intensity type")
    parser.add_argument("--dt_sim", type=int, default=5, help="Simulation timestep")
    parser.add_argument("--dt_env", type=int, default=10, help="Environment timestep")
    parser.add_argument("--yaw_step", type=float, default=5.0, help="Yaw step size")
